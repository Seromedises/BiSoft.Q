function [X,Y,Z] = BellowsSurface(obj,alpha_s,h,rcs1,Rs1,Rt1,h1,rcs2,Rs2,Rt2,stroke)
...
theta = linspace(0,pi/2-alpha_s,24)';
r1 = rcs1-Rs1*cos(theta);
z1 = Rs1*sin(theta);
r2 = linspace(Rt1,Rt2,2)';
z2 = h1+(r2-Rt1)*tan(alpha_s);
r3 = rcs2+Rs2*cos(flip(theta));
z3 = h-Rs2*sin(flip(theta));
...
r = NaN(size(rs,1)*ns,1);
z = NaN(size(zs,1)*ns,1);
