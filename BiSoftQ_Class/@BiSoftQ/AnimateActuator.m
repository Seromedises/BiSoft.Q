...
[alpha_b_vec,h_vec,rcvb_vec,Rvb_vec,Rt1_vec,h1_vec,rccb_vec,Rcb_vec,Rt2_vec] = C{:};
...
parfor (jj=1:2,NumWorkers)
    ...
    for ii=2:iter_max
        ...
        if ii==idxs(jj)
            ...
            [X_bell,Y_bell,Z_bell] = obj.Bellows.BellowsSurface(alpha_b_vec(ii,jj),h_vec(ii,jj), ...
                rcvb_vec(ii,jj),Rvb_vec(ii,jj),Rt1_vec(ii,jj),h1_vec(ii,jj),rccb_vec(ii,jj), ...
                Rcb_vec(ii,jj),Rt2_vec(ii,jj),x_vec(ii,jj));
...
for pp=1:obj.NPoints+1
    ...
    for jj=2:2*nb    
        plot3(X_bell_def((jj-1)*size(Z_bell_def,1)/(2*nb),:),Y_bell_def((jj-1)*size(Z_bell_def,1)/(2*nb),:),Z_bell_def((jj-1)*size(Z_bell_def,1)/(2*nb),:),'k-','LineWidth',1.5)
    end
    ...
    valley_indx_membr = size(X_membr,1)/(2*Np);
    for iii=1:2*Np
        idx = (iii-1)*valley_indx_membr+1;
    ...
    rho_v = sqrt(X_membr_frame(valley_indx_membr,:,pp,1).^2+Y_membr_frame(valley_indx_membr,:,pp,1).^2);
    
    R_bell_def = sqrt(X_bell_def.^2+Y_bell_def.^2);
    ...
    if strcmp(obj.type,"A")
        ...
        fill([R_bell_def(1,1) rho_c R_bell_def(end,1) flip(R_bell_def(:,1))'],[Z_bell_def(1,1) Z_membr_frame(1,:,pp,1) Z_bell_def(end,1) flip(Z_bell_def(:,1))'],'c','FaceAlpha',0.5)
        fill([-R_bell_def(1,1) -rho_c -R_bell_def(end,1) -flip(R_bell_def(:,1))'],[Z_bell_def(1,1) Z_membr_frame(1,:,pp,1) Z_bell_def(end,1) flip(Z_bell_def(:,1))'],'c','FaceAlpha',0.5)
    ...
    plot(R_bell_def(:,1),Z_bell_def(:,1),'k-')
    plot(-R_bell_def(:,1),Z_bell_def(:,1),'k-')

    H1 = plot(rho_c,Z_membr_frame(1,:,pp,1),'b');
    plot(-rho_c,Z_membr_frame(1,:,pp,1),'b')

    H2 = plot(rho_v,Z_membr_frame(valley_indx_membr,:,pp,1),'r');            
    plot(-rho_v,Z_membr_frame(valley_indx_membr,:,pp,1),'r')
...
for pp=idx_end_1:-1:1
    ...
    if strcmp(obj.type,"A")
        ...
        fill([R_bell_def(1,1) R_bell_def(:,1)' R_bell_def(end,1) -R_bell_def(end,1) -flip(R_bell_def(:,1))' -R_bell_def(1,1)],...
    ...
    elseif strcmp(obj.type,"B")
        ...
        fill(R_bell_def,Z_bell_def,'c','FaceAlpha',0.5)
...
for pp=1:obj.NPoints+1
    ...
    rho_v = sqrt(X_membr_frame(valley_indx_membr,:,pp,2).^2+Y_membr_frame(valley_indx_membr,:,pp,2).^2);
    ...
    H2 = plot(rho_v,Z_membr_frame(valley_indx_membr,:,pp,2),'r');            
    plot(-rho_v,Z_membr_frame(valley_indx_membr,:,pp,2),'r')
...
for pp=obj.NPoints+1:-1:1
    ...
    if strcmp(obj.type,"A")
        ...
        fill([R_bell_def(1,1) rho_c R_bell_def(end,1) flip(R_bell_def(:,1))'],[Z_bell_def(1,1) Z_membr_frame(1,:,pp,2) Z_bell_def(end,1) flip(Z_bell_def(:,1))'],'c','FaceAlpha',0.5)
        fill([-R_bell_def(1,1) -rho_c -R_bell_def(end,1) -flip(R_bell_def(:,1))'],[Z_bell_def(1,1) Z_membr_frame(1,:,pp,2) Z_bell_def(end,1) flip(Z_bell_def(:,1))'],'c','FaceAlpha',0.5)
    ...
    plot(R_bell_def(:,1),Z_bell_def(:,1),'k-','LineWidth',1.5)
    plot(-R_bell_def(:,1),Z_bell_def(:,1),'k-','LineWidth',1.5)
